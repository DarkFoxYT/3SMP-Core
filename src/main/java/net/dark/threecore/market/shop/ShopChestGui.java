...
public final class ShopChestGui {
    public Inventory buildSettings(ShopChestData data) {
        ...
        fill(inv);
        inv.setItem(11, button(Material.ITEM_FRAME, "<gradient:#1A2A4A:#f59e0b>Set Item</gradient>", List.of("<gray>Use your held item.</gray>", "<gray>Current:</gray> <white>" + data.itemName() + "</white>")));
        inv.setItem(13, button(Material.PAPER, "<gradient:#1A2A4A:#f59e0b>Set Price</gradient>", List.of("<gray>Current:</gray> <white>" + data.price() + "</white>")));
        inv.setItem(15, button(Material.CHEST, "<gradient:#1A2A4A:#f59e0b>Set Quantity</gradient>", List.of("<gray>Current:</gray> <white>" + data.quantity() + "</white>")));
    ...
    public Inventory buildBuy(ShopChestData data, int stock) {
        ...
        fill(inv);
        inv.setItem(13, button(Material.valueOf(data.itemType()), "<gradient:#1A2A4A:#f59e0b>" + data.itemName() + "</gradient>", List.of("<gray>Price per unit:</gray> <gold>" + data.price() + "</gold>", "<gray>Stock:</gray> <white>" + stock + "</white>", "<gray>Owner:</gray> <white>" + (data.owner() == null ? "None" : data.owner().toString()) + "</white>")));
